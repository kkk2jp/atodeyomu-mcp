...
> スコープは `tweet.read` `users.read` `offline.access` の 3 つだけを使います。`bookmark.read` は不要です。
...
`.env.example` をコピーして `.env` を作り、控えた値を入れます。
...
以降、トークンは MCP サーバーが自動でリフレッシュします。再認可が必要になるのは refresh token が失効したときだけです（その場合は `npm run auth` を再実行）。
...
`~/.mcp.json` に次のように登録します（パスは実環境に合わせてください）。
...
- `.env` とトークン（`~/.atodeyomu-mcp/tokens.json`）は秘密情報です。コミット・共有しないでください。
