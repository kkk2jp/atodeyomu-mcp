...
> 設計の詳細は [docs/DESIGN.md](./docs/DESIGN.md)、フロー図つきの仕様は [docs/SPEC.md](./docs/SPEC.md) を参照してください。動作確認の詳細手順は [docs/VERIFICATION.md](./docs/VERIFICATION.md)、Cowork スケジュールタスクで Notion に記録するまでの手順は [docs/COWORK_PIPELINE.md](./docs/COWORK_PIPELINE.md) を参照してください。
...
トークンを取得する認可スクリプトを一度だけ実行します。`CLIENT_ID` / `CLIENT_SECRET` は環境変数として渡します。

```bash
CLIENT_ID=控えたClientID CLIENT_SECRET=控えたClientSecret npm run auth
```

1. ターミナルに表示された認可 URL をブラウザで開きます。
2. X で承認すると `http://127.0.0.1:8787/callback` にリダイレクトされ、スクリプトが access / refresh token を取得します。
...
`~/.mcp.json` に次のように登録します。`CLIENT_ID` / `CLIENT_SECRET` は `env` で渡します（サーバーがトークンを自動リフレッシュする際に必要です）。`args` のパスは実環境の絶対パスに合わせてください（`realpath dist/index.js` で確認できます）。
...
```jsonc
{
  "mcpServers": {
    "atodeyomu": {
      "command": "node",
      "args": ["/absolute/path/to/atodeyomu-mcp/dist/index.js"],
      "env": {
...
2. `npm run build` 済みであることを確認します。
3. `~/.mcp.json` に登録した状態で Claude Code を起動します。
...
| 症状 | 対処 |
| --- | --- |
| `auth/setup.ts を再実行してください` と出る | refresh token が失効しています。`npm run auth` をやり直してください。 |
...
- 資格情報（Client ID / Secret）は環境変数で渡します（`~/.mcp.json` の `env`、認可時はコマンドのインライン指定）。
