...
```bash
git clone https://github.com/kkk2jp/atodeyomu-mcp.git
cd atodeyomu-mcp
npm install            # prepare スクリプトで自動ビルドされます
npm run auth -- --client-id xxx --client-secret yyy   # 認可（= node dist/index.js auth）
npm start              # サーバーをローカル起動（= node dist/index.js）
```

npm に公開する場合（`npx -y atodeyomu-mcp` を有効にする）:

```bash
npm login
npm publish            # prepare で dist をビルドし、files の dist のみ同梱して公開
```

変更をリリースするたびに `npm version patch|minor|major` でバージョンを上げてから `npm publish` します。
