...
- **引用元コンテンツの4パターンと本文要約**: ページ本文の要約（記事の要点を箇条書き＋結論でまとめた本文要約）の材料は、引用元ポストの内容に応じて次の4パターンがある。
  ...
  - **Xネイティブ記事**（引用元ポストが X の記事機能 `x.com/i/article/…` へのリンクを貼っている）: Premium+ で書ける長文記事で、上の長文ポスト（`note_tweet`、対応済み）とは別機能。X API に記事本文を返すエンドポイントが無く、Web からも認証必須のため本文は取得できない。`fetch_article` はこの URL を `status: "x_article"` で明示的に弾く（以前は通常 fetch 経路に入り「`ok` だが本文が実質空」という紛らわしい結果になっていた）。タスク側はフォールバック要約（引用コメント＋引用元本文から作る）とし、Notion の `Summary` の冒頭に「【X記事・本文未取得】」を付けて一覧画面で識別できるようにする（`cowork-prompt.md` 手順 2 に記載済み）。
