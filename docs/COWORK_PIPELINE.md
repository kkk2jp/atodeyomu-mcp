...
2. Claude がデータベースを作成すると、データベースのURLと**データソースURL**（`collection://xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` の形式）が返ってくる。このデータソースURLを控えておく。これが §4 で使うプレースホルダの実値になる。
...
Claude Code の `schedule` スキル、または `scheduled-tasks` MCP（`create_scheduled_task`）でタスクを登録する。スケジュールタスクは **このアプリ（Claude Code）が起動している間に実行**され、ローカルの `~/.mcp.json` に登録された MCP サーバー（atodeyomu, notion）にそのままアクセスできる。クラウド専用エージェントではないので、ローカルの OAuth トークン（`~/.atodeyomu-mcp/tokens.json`）に依存する atodeyomu-mcp の前提（[docs/DESIGN.md](./DESIGN.md) §8「ローカルタスク前提」）と矛盾しない。
