...
- OAuth 2.0 Authorization Code + PKCE による認可スクリプト（`auth/setup.ts`）。
...
atodeyomu-mcp/
├── auth/
│   └── setup.ts              # 一度だけ手動実行する OAuth 認可スクリプト
├── src/
│   ├── index.ts              # MCP サーバー本体（stdio transport, ツール登録）
...
### 5.1 `auth/setup.ts`（認可スクリプト）

一度だけ手動実行し、トークンを取得・保存する。

- `CLIENT_ID` / `CLIENT_SECRET` を環境変数（`process.env`）から読み込む。認可時はコマンドのインライン指定で渡す（`CLIENT_ID=xxx CLIENT_SECRET=yyy npm run auth`）。
...
- `loadTokens()`: `~/.atodeyomu-mcp/tokens.json` を読み込む。存在しなければ「`auth/setup.ts` を実行してください」という旨のエラーを投げる。
...
| 認証エラー（401 / refresh token 失効） | 「`auth/setup.ts` を再実行してください」と明示したメッセージを返す。 |
...
- `CLIENT_ID` / `CLIENT_SECRET` は環境変数から読み込む。サーバー起動時は `~/.mcp.json` の `env`、認可スクリプト実行時はコマンドのインライン指定で渡す。
- トークンファイルは `chmod 600`。
...
- `~/.mcp.json` に登録し、Claude Code から `get_quoted_posts` を呼び出して動作確認する（手順は README）。
