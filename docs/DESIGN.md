...
- `.env` から `CLIENT_ID` / `CLIENT_SECRET` を読み込む。
...
- シークレット・トークンを **絶対にコミットしない**。`.gitignore` に `.env` と（保険として）`tokens.json` 系を含める。
- `CLIENT_ID` / `CLIENT_SECRET` は `.env` から読み込む。`.env.example` を用意する。
- トークンファイルは `chmod 600`。
