...
| パラメータ | 型 | 必須 | デフォルト | 範囲 |
| --- | --- | --- | --- | --- |
| `limit` | number | 任意 | なし | 1〜50。指定すると MCP が内部で全ページを取得し、**古い順（id 昇順）に先頭 `limit` 件だけ**返す（後述）。`max_results` / `pagination_token` とは併用しない |
| `max_results` | number | 任意 | 20 | 1〜100（`limit` 未指定の従来モードで有効） |
| `since_id` | string | 任意 | カーソル値 | 取りこぼし時に手動で取得開始位置を巻き戻すための上書き |
| `pagination_token` | string | 任意 | なし | 前回のレスポンスの `next_token`。差分が `max_results` を超えるときに続きのページを取るために渡す（`limit` 未指定の従来モードで有効） |
...
`url` は API から返ってこないので、`username` と `id` を組み立てて生成します。引用元のメディアが無ければ `media` は空配列です。`text` / `quoted_post.text` は、通常の文字数上限を超える「長いポスト」の場合、切り詰められた `text` ではなく全文が入る `note_tweet.text` を優先して使います。`newest_seen_id` は取得したタイムライン全体（引用以外も含む）の最大 id で、全件を無事 Notion に保存できたら、この値を `commit_cursor` に渡します。`next_token` は続きのページがあるときだけ値が入り、無ければ `null` です（詳細は §6.2）。`has_more` は「返した分の先にまだ未処理の引用ポストが残っているか」を示します。

**`limit` モード**: `limit` を指定すると、MCP が内部で全ページを取得したうえで引用ポストを id 昇順にソートし、**古い順に先頭 `limit` 件だけ**を `posts` に入れて返します（ページ送りは MCP 内部で完結）。呼び出し側は 1 回呼ぶだけでよく、差分全件をコンテキストに載せずに「処理すべき古い N 件」だけを受け取れます。このとき `next_token` は常に `null` で、続きがあるかは `has_more` で判断します（`limit` を超える引用ポストが残っていれば `true`）。従来モード（`limit` 未指定）では `has_more` は `next_token` の有無と同値です。
...
| `x_article` | リダイレクト先が X ネイティブ記事（`x.com/i/article/…`）だった。認証必須のため本文は取得できない | フォールバック |
| `not_article` | 取得できたが本文抽出できなかった（画像・動画のみ、非 HTML 等） | フォールバック |
| `fetch_failed` | ネットワークエラー・非 HTTP・ペイウォール・タイムアウト等 | フォールバック |

`ok` 以外のとき `text` は空文字です。失敗（`fetch_failed` / `not_article`）は「記事が取れなかった」だけで **パイプラインを止めるエラーではありません**。呼び出し側は引用コメント＋引用元本文からの要約にフォールバックします。`x_post` は入れ子引用の別ポストを弾くだけで、その **孫ポストの本文までは取りに行きません**（2 段展開はしない方針）。`x_article` は X ネイティブの「記事」機能（Premium+ の長文記事）で、通常の長文ポスト（`note_tweet`）とは別物です。X API に記事本文を返すエンドポイントが無く、Web から fetch しても認証必須でほぼ空のシェルページしか返らないため、紛らわしい「本文が実質空の `ok`」にせず明示的に弾いています（`detail` に理由が入ります）。
...
### 6.2 大量の差分を取り切る 2 つの経路

差分が溜まって 1 回の呼び出しでは取りきれない場合、取得側には 2 つの経路があります。

**`limit` モード（推奨）**: `limit` を指定すると、MCP 側が内部でページ送りを繰り返して全差分を集め、引用ポストを id 昇順にソートして**古い順に先頭 `limit` 件だけ**を返します。ページ送りループは MCP の中で完結するので、呼び出し側は 1 回呼ぶだけで「処理すべき古い N 件」と `has_more`（残りがあるか）を受け取れます。ねらいは、差分全件を呼び出し側のコンテキストに載せないこと——大量バックログがあっても、実際に処理する N 件だけがコンテキストに入ります。呼び出し側は古い順に処理し、処理後に `commit_cursor` を次のように呼びます。

- 全件処理でき `has_more: false`（今回でタイムライン全体を見終えた）→ `newest_seen_id` で確定。
- 全件処理でき `has_more: true`（返した N 件の先にまだ残りがある）→ 返ってきた `posts` の**最後（最も新しい）の id** で確定。残りは次回に回る。
- 保存エラーで中断 → 「安全に保存できた最後の id」で確定（1 件も保存できなければ確定しない）。

**従来モード（`pagination_token` によるページ送り）**: `limit` を指定しない場合は、X API の `pagination_token` でページ送りする仕組みをそのまま使えます。レスポンスの `next_token` を次回呼び出しの `pagination_token` に渡すことで、同じ `since_id` 境界内の続きのページを取得できます。
...
どちらも初回実行時（`since_id` 未指定かつ `cursor.json` が無い）や、`cursor.json` を削除してリセットした直後も同じ流れで使えます。ページ送りを一切せず 1 回だけ呼ぶ運用でもよく、その場合は取得できなかった古い引用ポストは次回実行（`commit_cursor` で確定した位置からの差分取得）で拾われます。
