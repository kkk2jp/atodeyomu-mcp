...
トークンは `auth/setup.ts` を **一度だけ手動実行** して取得します。同じ PC 上で繰り返し動かす「ローカルタスク」前提です。

```mermaid
sequenceDiagram
    autonumber
    participant U as あなた
    participant S as auth/setup.ts
    participant B as ブラウザ
    participant X as X 認可サーバー
    participant TK as ~/.atodeyomu-mcp/tokens.json

    U->>S: npm run auth で実行
...
| 認証エラー (401 / refresh token 失効) | 「`auth/setup.ts` を再実行してください」 |
